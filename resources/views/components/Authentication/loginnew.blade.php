<div class="z-[100] hidden overflow-x-hidden overflow-y-auto fixed inset-0  outline-none focus:outline-none justify-center items-center" id="loggins-id">
    <div class="relative w-full sm:w-4/5 my-6 mx-auto max-w-3xl">
      <!--content-->
      <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full sm:w-4/5 bg-white outline-none focus:outline-none">
        <!--header-->
        <hr class="text-black">
     <div class="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t ">
        <div class="pl-16 md:pl-40"> <svg id="Component_6_1" data-name="Component 6 – 1" xmlns="http://www.w3.org/2000/svg" width="200" height="41.21" viewBox="0 0 110 21.21">
     ...
     <div>
      <button class="pl-5 md:pl-16 text-2xl text-pink" onclick="toggleloggins('loggins-id')">
     X
      </button></div>
    </div>
    <!--body-->
<form method="post" action="/registerstorel">
  @csrf
<div class="text-pink pt-2 pl-14 text-xl ">Accedi</div>
<div class="text-center ">
<div class="pt-10">
<label for="email"></label>
<input type="text" name="email" placeholder="Email" required class="border border-black w-4/5 h-10 bg-white">
</div>
<div class="pt-10">
<label for="password"></label>
<input type="password" name="password" placeholder="Password" autocomplete="current-password" required="" id="id_passwordd"  class="border border-black w-4/5 h-10 bg-white">
<i class="far fa-eye" id="togglePasswordd" style="margin-left: -30px; cursor: pointer;"></i>
<div class="underline pt-5 pl-20 md:pl-72"><a href="">Hai dimenticato la password?</a></div>
<script>
  const togglePassword = document.querySelector('#togglePasswordd');
  const password = document.querySelector('#id_passwordd');
...
<input type="submit" value="Accedi" class="text-white hover:bg-white bg-pink hover:text-pink border-2 mt-5 mb-10 rounded-full w-72 h-9 sm:w-4/5 sm:h-10">
</div>
</form>
<hr>
<div class="inline-flex text-pink pl-24 md:pl-52 mb-5 mt-3">
    <div >Non hai un account?</div>
      <div>  <button class="underline pl-2" type="button" onclick="toggleregister('register-id');toggleloggins('loggins-id')">
        Registrati</button></div>    
 </div>
</div>
      </div>
    </div>
<div class="hidden opacity-25 fixed inset-0 z-40 bg-black" id="loggins-id-backdrop"></div>
  <script type="text/javascript">
    function toggleloggins(logginsID){
      document.getElementById(logginsID).classList.toggle("hidden");
      document.getElementById(logginsID + "-backdrop").classList.toggle("hidden");
      document.getElementById(logginsID).classList.toggle("flex");
      document.getElementById(logginsID + "-backdrop").classList.toggle("flex");
    }
  </script>
