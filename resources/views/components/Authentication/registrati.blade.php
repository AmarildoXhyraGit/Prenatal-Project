<div class="z-[100] hidden overflow-x-hidden overflow-y-auto fixed inset-0  outline-none focus:outline-none justify-center items-center" id="register-id">
  <div class="relative w-full sm:w-4/5 my-6 mx-auto max-w-3xl">
    ...
    <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full sm:w-4/5 bg-white outline-none focus:outline-none">
      ...
      <div class="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t ">
          <div class="sm:pl-40 pl-10 "> <svg id="Component_6_1" data-name="Component 6 – 1" xmlns="http://www.w3.org/2000/svg" class="w-32 md:w-52" viewBox="0 0 110 21.21">
       ...
       <div>
        <button class="pl-14  text-2xl text-pink" onclick="toggleregister('register-id')">
       X
        </button></div>
      </div>
        <!--body-->
        <form action="/register" method="post">
          @csrf
  <div class="text-pink pt-2 pl-14 text-xl ">Registrati</div>
  <div class="text-center ">
    <div class="pt-10">
      <label for="name"></label>
      <input type="text" name="name" placeholder="name" required class="border border-black w-4/5 h-10" value="{{old('name')}}">
      @error('name')
  ...
  <input type="text" name="email" placeholder="Email" required class="border border-black w-4/5 h-10">
  @error('email')
  <p class="text-red-600">{{$message}}</p>
@enderror
  </div>
  <div class="pt-10">
    ...
    <input type="password" name="password" placeholder="Password" autocomplete="current-password" required="" id="id_password"  class="border border-black w-4/5 h-10 bg-white">
  ...
  <input type="submit" value="Registrati" class="text-white hover:bg-white bg-pink hover:text-pink border-2 mt-5 mb-10 rounded-full w-72 h-9 sm:w-4/5 sm:h-10">
  </div>
    </form>
  <hr>
  <div class="inline-flex text-pink text-center pl-52 mb-5 mt-3">
  <div >Hai già un account?</div>
    <div >  
        <button type="button" class="underline pl-2 " onclick="toggleloggins('loggins-id');toggleregister('register-id')">
        Accedi
      </button>  
</div>
</div>
        ...
        <!--footer-->   
  <div class="hidden opacity-25 fixed inset-0 z-40 bg-black" id="register-id-backdrop"></div>
  <script type="text/javascript">
    function toggleregister(registerID){
      document.getElementById(registerID).classList.toggle("hidden");
      document.getElementById(registerID + "-backdrop").classList.toggle("hidden");
      document.getElementById(registerID).classList.toggle("flex");
      document.getElementById(registerID + "-backdrop").classList.toggle("flex");
    }
  </script>